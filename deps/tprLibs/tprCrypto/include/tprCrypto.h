...
struct CheckSum64{
    u8_t data[ CHECKSUM64_LEN ] = {0};  //-- 显式初始化，很有效
};

//--- checksum128 的类型，实体是个 固定的 二进制数据串。 128-bits ／ 16-bytes
#define CHECKSUM128_LEN 16 
struct CheckSum128{
    u8_t data[ CHECKSUM128_LEN ] = {0};  //-- 显式初始化，很有效
};


//----------- check_sum.cpp ------------//
//--- 64 --
CheckSum64 mk_checksum64_old( u8_t *_buf, size_t _len ); //-- 已被新版替代，不推荐使用
...
//--- 128 --
CheckSum128 mk_checksum128_old( u8_t *_buf, size_t _len ); //-- 已被新版替代，不推荐使用
