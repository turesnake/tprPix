...
def is_2_cir_collide( a_,  b_ ):
    ...
    ADJUST = 10 # half mapent
    sum_of_rads = a_.radius + b_.radius - ADJUST
...
# param: a_ Cir
# param: b_ Cir
def is_2_cir_collide_for_Mid2( a_,  b_ ):

    offX = math.fabs( a_.dpos.x - b_.dpos.x )
    offY = math.fabs( a_.dpos.y - b_.dpos.y )

    return (False if ((offX>64) and (offY>64)) else True )
    #------------------#
...
def detect_is_collide( cir_, cirs_ ):
    for be in cirs_:
        if( is_2_cir_collide(cir_, be) ):
            return True
        #if( is_2_cir_collide_for_Mid2(cir_, be) ):
        #    return True
    return False
    #------------------#
