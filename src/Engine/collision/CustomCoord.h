 ...
 * ------------
 */
#ifndef TPR_CUSTOM_COORD_H
#define TPR_CUSTOM_COORD_H
...
class CustomCoord{
public:
    CustomCoord( const glm::dvec2 &x_, const glm::dvec2 &y_ ):
        xP(x_),
        yP(y_),
    ...
    inline glm::dvec2 calc_innDPos( const glm::dvec2 &outDPos_ )const noexcept{
        return glm::dvec2{
            (yP.y * outDPos_.x - yP.x * outDPos_.y) / this->denominator,
            (xP.x * outDPos_.y - xP.y * outDPos_.x) / this->denominator
        };
    }
