 ...
 */
void Altitude::set_old( const IntVec2 &_ppos, const EcoSys *_ecosysPtr ){
...
void Altitude::set( float _altiVal_from_gpgpu ){
    ...
    if( this->val < 0 ){ //- under water
        tmpLvl = static_cast<int>( _altiVal_from_gpgpu/waterStep);
        if( tmpLvl < -5 ){ //- 抹平 水域底部
            tmpLvl = -5;
        }
    }else{ //- land
        tmpLvl = static_cast<int>( _altiVal_from_gpgpu/landStep);
        if( tmpLvl > 2 ){ //- 抹平 陆地高区
            tmpLvl = 2;
        }
    }
