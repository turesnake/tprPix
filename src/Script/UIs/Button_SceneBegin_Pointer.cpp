 ...
 * -----------------------------------------------------------
 */
void Button_SceneBegin_Pointer::init_in_autoMod(  GameObj *_goPtr,
                                                const IntVec2 &_mpos ){
    ...
    goPtr->RenderUpdate = std::bind( &Button_SceneBegin_Pointer::OnRenderUpdate, &button_sceneBegin_pointer, _1 );   
    goPtr->LogicUpdate  = std::bind( &Button_SceneBegin_Pointer::OnLogicUpdate,  &button_sceneBegin_pointer, _1 );
    
    //-------- actionSwitch ---------//
    goPtr->actionSwitch.bind_func( std::bind( &Button_SceneBegin_Pointer::OnActionSwitch, &button_sceneBegin_pointer, _1, _2 ) );
    goPtr->actionSwitch.signUp( ActionSwitchType::Move_Idle );
            //- 当前 只有一种动画，就是永久待机...

    //-------- go self vals ---------//
    goPtr->species = Button_SceneBegin_Pointer::specId;
    ...
    //-------- animFrameSet／animFrameIdxHandle/ goMesh ---------//
        ...
        GameObjMesh &rootGoMeshRef = 
                goPtr->creat_new_goMesh("root", //- gmesh-name
                                        ...
                                        1.0,  //- off_z  通过此值来确保，archive 在 pointer 后方。目前这个实现很不好...
                                        true, //- isVisible
                                        false, //- isCollide
                                        false //- isFlipOver
                                        );
        ...
        rootGoMeshRef.getnc_animFrameIdxHandle().bind_idle( 0 ); //- "pointer"
 ...
 * -----------------------------------------------------------
 */
void Button_SceneBegin_Pointer::OnRenderUpdate( GameObj *_goPtr ){
    //=====================================//
    //            ptr rebind
    //-------------------------------------//
    rebind_ptr( _goPtr );
 ...
 * -----------------------------------------------------------
 */
void Button_SceneBegin_Pointer::OnLogicUpdate( GameObj *_goPtr ){
    // 什么也没做...
}
 ...
 * -- 会被 动作状态机 取代...
 */
void Button_SceneBegin_Pointer::OnActionSwitch( GameObj *_goPtr, ActionSwitchType _type ){
    // 暂时什么也没做...
}
