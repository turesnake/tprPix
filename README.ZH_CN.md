...
## 支持哪款编译器
**Clang** 是最推荐使用的编译器，即便是在 **Win10** 平台。
确保你所下载和安装的 **Clang** 版本，支持 **C++17**。
...
脚本 `unix_release.sh` 将会自动化地帮我们完成程序的 **build** 和剩余本地化部署工作。
你也可以选择运行另一份脚本：`unix_debug.sh`， 正如名字所示，它将生存一份 **DEBUG** 版的程序。

现在，所有的安装工作都已经 **完成** 了!!!

你可以在目录： **.../app/build/publish/** 中找到可执行文件：**tprpix**。

双击它，开始我们的游戏!!!


### Ubuntu 
在 **Ubuntu** 上的安装其实与 **Mac OS X** 相似。
...
然后，启动 **Visual Studio 2019**，点选初始界面中的 `Open a local folder` 按钮，在弹出的窗口中，选择 本程序的根目录。 ->
...
你可以在目录 **.../app/build/publish/ folder** 中，找到可执行文件 **tprpix.exe**，
它就是我们的程序本体。
...
考虑到这是一款动作类游戏，作者还是推荐大家使用手柄来游戏。（ Xbox360 或其他山寨版都支持 ）
