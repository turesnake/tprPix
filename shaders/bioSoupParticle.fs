...
// origin colors
vec3 particle_light = vec3( 0.784, 0.784, 0.784 ); // 200
vec3 particle_mid   = vec3( 0.588, 0.588, 0.588 ); // 150
vec3 particle_dark  = vec3( 0.392, 0.392, 0.392 ); // 100
...
vec3 change_color( in vec3 val_ ){
    if(       closeEnough( val_, particle_light ) ){ return bioSoupColorTable.particle_light.rgb;
    }else if( closeEnough( val_, particle_mid   ) ){ return bioSoupColorTable.particle_mid.rgb;
    }else if( closeEnough( val_, particle_dark  ) ){ return bioSoupColorTable.particle_dark.rgb;
    }else{
